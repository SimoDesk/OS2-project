...
void copiaMatrice(cmatrix* dest, cmatrix part) {
    free(dest->id); /*  Libero lo spazio di memoria dedicato all'id della matrice "dest", che ora non serve più */
    dest->id = malloc(strlen(part.id) + 1); /*  Alloco lo spazio necessario per memorizzare l'id della matrice "part" */
    if(dest->id == NULL) error("Errore allocazione memoria dell'id di una matrice (types.c)");
    strcpy(dest->id, part.id); /*  Copio l'id della matrice "part" nella matrice "dest" */
