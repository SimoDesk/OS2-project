...
void getMatrix(cmatrix* m, char* content, char* id, int dimensione) {
    ...
    int n_righe = 0;
    int c = 0;
    while(copiaContent[c] != '\0') { /* Calcolo il numero di righe nel file contando il numero di caratteri '#' */
        if(copiaContent[c] == '#') n_righe++;
        c++;
    }

    c = 0; /* Resetto il contatore per poterlo riutilizzare */

    char** righe = malloc(n_righe * sizeof(char*)); /* Alloco dinamicamente lo spazio di memoria per un array di puntatori a stringhe che conterrà le righe del file */
    char* riga = strtok(copiaContent, "#"); /* Inizializzo la variabile "riga" con la prima riga del file, separando le righe con il carattere "#"  */
    while (riga != NULL) {
        righe[c] = malloc(strlen(riga)+1); /* Alloco dinamicamente lo spazio di memoria per la riga corrente */
        if(righe[c] == NULL) error("Errore allocazione memoria in getMatrix"); /* Se dovessero esserci problemi con l'allocazione della memoria, un errore fermerebbe l'esecuzione */
        strcpy(righe[c], riga);  /* Copio la riga corrente nell'array di puntatori a stringhe "righe" */
        c++;    /* Incremento il contatore delle righe */
        riga = strtok(NULL, "#");   /* Proseguo nel controllo delle righe, andando alla prossima chiamando nuovamente strtok() */
    }
    ...
    for(c = 0; c < n_righe; c++) { /* Per ogni riga del file, controllo se l'id della matrice corrisponde a quello passato in input */
        if(strncmp(righe[c], "define", 6) == 0) {  /* Finché ci sono righe nel file, controllo se i primi 7 caratteri della riga corrente corrispondono all'identificatore "#define" */
            ...
            getSubstring(tempStr, righe[c], 6, strlen(righe[c])-5); /* Copio nella stringa "tempStr" il contenuto del resto della riga (tutto meno l'identificatore) */
            ...
            if(strcmp(idFound, id) == 0) {  /* Controllo se l'id della matrice corrisponde a l'omonimo parametro passato in input */
                ...
                char* pos_start = strchr(righe[c], '[');    /* Dando per scontato che la sintassi del vettore preveda un solo carattere '[', ne ricavo la posizione con il metodo strchr() e la memorizzo nel parametro "start" */
                if (pos_start == NULL) {    /* Se il carattere '[' non viene trovato, significa che la sintassi del vettore non è corretta */
                    fprintf(stderr, "Errore: carattere '[' non trovato nel vettore init\n");    /* Qualora il carattere '[' non dovesse essere stato trovato nella riga */
                    exit(1);                                                                    /* il programma verrebbe interrotto con un errore */
                }
                int start = (int)(pos_start - righe[c])+1;    /* Dando per scontato che la sintassi del vettore preveda un solo carattere '[', ne ricavo la posizione con il metodo strchr() e la memorizzo nel parametro "start" */
    
                char* pos_end = strchr(righe[c], ']');  /* Dando per scontato che la sintassi del vettore preveda un solo carattere ']', ne ricavo la posizione con il metodo strchr() e la memorizzo nel parametro "end" */
                if (pos_end == NULL) {  /* Se il carattere ']' non viene trovato, significa che la sintassi del vettore non è corretta */
                    fprintf(stderr, "Errore: carattere ']' non trovato nel vettore init\n");    /* Qualora il carattere ']' non dovesse essere stato trovato nella riga */
                    exit(1);                                                                    /* il programma verrebbe interrotto con un errore */
                }
                int end = (int)(pos_end - righe[c]);    /* Dando per scontato che la sintassi del vettore preveda un solo carattere ']', ne ricavo la posizione con il metodo strchr() e la memorizzo nel parametro "end" */
                ...
                matrixContent[0] = '\0';
                getSubstring(matrixContent, righe[c], start, end);  /* Con il metodo getSubstring() creo in "matrixContent" una copia della sottostringa corrispondente al vettore dichiarato dopo "#define" con il corretto id */
                ...
                char* vettoreComplesso; /* Il parametro "vettoreComplesso" conterrà i singoli vettori di numeri complessi della matrice, che verranno analizzai con strtok */
                char* numComplesso; /* Il parametro "numComplesso" conterrà i singoli numeri complessi dei vettori della matrice, che verranno analizzai con strtok */

                char** vettori = malloc(sizeof(char*) * dimensione);    /* Alloco dinamicamente lo spazio di memoria per un array di puntatori a stringhe che conterrà i vettori della matrice, che sono memorizzati in matrixContent */

                vettoreComplesso = strtok(matrixContent, ")");    /* Utilizzo il metodo strtok_r() con divisore ")" per poter controllare singolarmente i vettori nella matrice  */
                while(vettoreComplesso != NULL) {   /* Finché ci sono vettori nella matrice, utilizzo il metodo strtok_r() con divisore ")" per poter controllare singolarmente i vettori nella matrice */
                    ...
                    vettori[countVettori] = malloc(strlen(vettoreComplesso) + 1);   /* Alloco dinamicamente lo spazio di memoria per il vettore corrente, che è una stringa */
                    if (vettori[countVettori] == NULL) {
                        fprintf(stderr, "Errore allocazione vettore %d della matrice %s in getMatrix\n", countVettori, id); /* Controllo che l'allocazione della memoria sia andata a buon fine */  
                        exit(1);                                                                                            /* Se no, stampo un messaggio di errore e termino l'esecuzione */
                    }
                    strcpy(vettori[countVettori], vettoreComplesso);    /* Copio il vettore corrente nella posizione corretta dell'array di puntatori a stringhe vettori */
                    
                    countVettori++; /* Incrementando "countVettori", tengo il conto dei vettori trovati nella matrice */
                    vettoreComplesso = strtok(NULL, ")");   /* Proseguo nel controllo dei vettori nella matrice, andando al prossimo chiamando nuovamente strtok() */
                }

                int l;  /* Dichiaro una variabile "l" che verrà utilizzata per scorrere i vettori della matrice */
                for(l = 0; l< dimensione; l++) {
                    j = 0;
                    char* numComplesso = strtok(vettori[l], ","); /* Per ogni vettore nella matrice, utilizzo il metodo strtok_r() con divisore "," per poter controllare singolarmente i numeri complessi nei vettori */
                    while(numComplesso != NULL) {
                        ...
                        numComplesso = strtok(NULL, ",");      /* Proseguo nel controllo dei numeri complessi nel vettore, andando al prossimo chiamando nuovamente strtok() */
                    }
                ...
                int k;
                for(k = 0; k < dimensione; k++) {   /* Libero lo spazio di memoria dedicato ai vettori della matrice, che ora non serve più */
                    free(vettori[k]);   /* Libero lo spazio di memoria dedicato al vettore corrente, che ora non serve più */
                }
                free(vettori);  /* Libero lo spazio di memoria dedicato all'array di puntatori a stringhe che conteneva i vettori della matrice, che ora non serve più */
        ...
        if(found) break;  /* Seleziono solo la prima riga corretta che incontro, ignorando altre eventuali */
    }
    free(copiaContent); /* Libero lo spazio di memoria dedicato alla copia del contenuto del file, che ora non serve più */
    c = 0; /* Resetto il contatore per poterlo riutilizzare */
    for(c = 0; c < n_righe; c++) { /* Libero lo spazio di memoria dedicato alle righe del file, che ora non serve più */
        free(righe[c]); /* Libero lo spazio di memoria dedicato alla riga corrente, che ora non serve più */
    }
    free(righe); /* Libero lo spazio di memoria dedicato all'array di puntatori a stringhe che conteneva le righe del file, che ora non serve più */
